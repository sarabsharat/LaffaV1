  ...
  return (
    <div className="contact-container">
      ...
      <form onSubmit={onSubmit} className="contact-form">
        ...
        <button 
          type="submit" 
          className={`submit-btn ${resultStatus === 'success' ? 'active' : ''}`} 
          disabled={resultStatus === 'sending' || resultStatus === 'success'}
        >
            <p className="btn-text">{resultStatus === 'success' ? 'Thanks' : 'Send Message'}</p>
      ...
      <span className={`form-result ${resultStatus}`}>{result}</span>
    </div>
  );
}
